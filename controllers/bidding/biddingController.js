...
exports.biddingHistory = async (req, res) => {
  ...
  await Bidders.find({}).populate("biddingProductId").then(bidders=>{
    totalProducts=bidders;

    /* for(let i=0;i<bidders.length;i++){
      if(bidders[i].bid_amount>maxBiddingamount){
        maxBiddingamount=bidders[i].bid_amount;
      }
    } */
  ...
  await Bidders.find({userId: param}).populate("biddingProductId").then( (biddingProductDetails)=>{
    ...
    if(biddingProductDetails.length>0){
     // console.log(JSON.stringify(totalProducts)+ " TotaklProducts");
     
      for(let i=0;i<biddingProductDetails.length;i++){
        let won=true;
       
        
        let timeDiff = moment(moment(biddingProductDetails[i].biddingProductId.endTime,"YYYY-MM-DD")).diff(moment().format("YYYY-MM-DD"), 'seconds');
        if(timeDiff<=0){
          let filterBiddProdById=[];
          for(let k=0;k<totalProducts.length;k++){
        
           
            if(totalProducts[k].biddingProductId._id.equals(biddingProductDetails[i].biddingProductId._id)){
             
              filterBiddProdById.push(totalProducts[k]);
            }
          }
     
          for(let j=0;j<filterBiddProdById.length;j++){
            if(biddingProductDetails[i].bid_amount<filterBiddProdById[j].bid_amount){
              biddingProductDetails[i].bid_status="LOST";
              won=false;
              break;
            }
          }
          ...
          if(won){
            biddingProductDetails[i].bid_status="WINNER";
          }

        }else{
          biddingProductDetails[i].bid_status="INPROCESS";
        }
